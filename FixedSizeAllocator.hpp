...
public:
  explicit FixedSizeAllocator(int _size)
      : m_size(_size), m_objects(m_size), m_allocation(m_size),
  ...
  bool canAllocate() const { return m_ptrAlloc != m_allocation.rend(); }
  ...
  void deallocate(T *_obj) { m_allocation[++m_ptrAlloc] = _obj; }
