...
template <typename T>
class LSDRadixSort {
    static_assert(std::is_integral<T>::value, "Radix sort cannot be applied to non integral number");
    static_assert(std::is_unsigned<T>::value, "Radix sort cannot be applied to non unsigned number (for now)");
  ...
  public:
    ...
    static void sort(InputIterator _begin, InputIterator _end) {
        ...
        for (int i = 0; i < static_cast<int>(getMaxSetByte(*std::max_element(_begin, _end))); ++i) {
            for (auto& b : buckets) {
                b.clear();
            };
  ...
  public:
    ...
    static void sort(RandomIterator _begin, RandomIterator _end) {
        ...
        std::vector<std::tuple<RandomIterator, RandomIterator, int>> stack =
            {std::make_tuple(_begin, _end, getMaxSetByte(*std::max_element(_begin, _end)))};
        while (!stack.empty()) {
...
class CountingMSDRadixSort {
  ...
  public:
    ...
    static void sort(const RandomIterator _begin, const RandomIterator _end) {
        ...
        std::vector<std::tuple<RandomIterator, RandomIterator, int>> stack = {std::make_tuple(_begin, _end, sizeof(T) - 1)};
        while (!stack.empty()) {
            auto[begin, end, i] = stack.back();
            stack.pop_back();
            const int power = 8 * i;
            ...
            for (int b = 0; b < base - 1; ++b) {
                if (nbValue[b] > 1) {
                    stack.emplace_back(begin + bucketIndices[b], begin + bucketIndices[b + 1], i - 1);
                }
            }
            if (nbValue[base - 1] > 1) {
