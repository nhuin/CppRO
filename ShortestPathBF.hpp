...
template <typename G> class ShortestPathBellmanFord {
public:
  ...
  explicit ShortestPathBellmanFord(const G &_graph)
      : m_graph(&_graph),
        ...
        m_inQueue(m_graph->getOrder(), false), m_count(m_graph->getOrder(), 0)
  ...
  Graph::Path getShortestPath_v1(const int _s, const int _t) {
    ...
    for (const auto &edge : m_graph->getEdges()) {
      if (m_distance[edge.first] + m_graph->getEdgeWeight(edge) <
