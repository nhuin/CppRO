...
template <typename T, typename Comparator = std::less<T>> class FibonacciHeap {
public:
  struct Node {
    T value;
  ...
  explicit FibonacciHeap(int _maxSize, const Comparator &_comp = Comparator())
      : m_heap(nullptr), m_min(nullptr), m_comparator(_comp),
        m_allocator(_maxSize), m_nodesAtRank(_maxSize) {}

  ~FibonacciHeap() { m_allocator.deallocateAll(); }
  ...
  bool empty() const { return !m_heap; }
  ...
  const T &top() { return m_min->value; }
