...
#if (defined _MSC_VER && _MSC_VER >= 1300) || (defined __GNUC__ && __GNUC__ >= 4 && __GNUC_MINOR__ >= 3) || (defined __clang__ && __clang_major__ >= 3)
...
#define INTERNAL_CATCH_TESTCASE2(TestName, ...)                                                                                    \
    ...
    namespace {                                                                                                                    \
    Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(&TestName, CATCH_INTERNAL_LINEINFO, Catch::NameAndDesc(__VA_ARGS__)); \
    }                                                                                                                              \
...
#define INTERNAL_CATCH_TESTCASE2(TestName, Name, Desc)                                                                            \
    ...
    CATCH_INTERNAL_SUPPRESS_ETD_WARNINGS                                                                                          \
    namespace {                                                                                                                   \
    Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(&TestName, CATCH_INTERNAL_LINEINFO, Catch::NameAndDesc(Name, Desc)); \
    }                                                                                                                             \
    CATCH_INTERNAL_UNSUPPRESS_ETD_WARNINGS                                                                                        \
    static void TestName()
...
class BinaryExpression : public DecomposedExpression {
  public:
    ...
    virtual void reconstructExpression(std::string& dest) const CATCH_OVERRIDE {
        ...
        char delim = lhs.size() + rhs.size() < 40 && lhs.find('\n') == std::string::npos && rhs.find('\n') == std::string::npos ? ' ' : '\n';
...
Colour::Colour(Code _colourCode)
    : m_moved(false) { use(_colourCode); }
Colour::Colour(Colour const& _other)
    : m_moved(false) { const_cast<Colour&>(_other).m_moved = true; }
Colour::~Colour() {
    if (!m_moved)
        use(None);
}
