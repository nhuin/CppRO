...
template <typename G> class ShortestPath {
public:
  explicit ShortestPath(const G &_graph)
      : m_graph(&_graph),
        m_distance(m_graph->getOrder(), std::numeric_limits<int>::max()),
        m_parent(m_graph->getOrder(), -1), m_color(m_graph->getOrder(), 0),
  ...
  // Copy
  ShortestPath(const ShortestPath &_other) : ShortestPath(*_other.m_graph) {}
  ...
  ShortestPath(ShortestPath &&_other) noexcept
      : m_graph(std::move(_other.m_graph)),
        ...
        m_color(std::move(_other.m_color)), m_handles(m_graph->getOrder()),
        m_heap(m_graph->getOrder(),
               [=](const Graph::Node __u, const Graph::Node __v) {
                 return m_distance[__u] < m_distance[__v];
               }) {}
