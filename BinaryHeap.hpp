...
template <typename T, typename Comparator = std::less<T>> class BinaryHeap {

public:
  class Handle {
    explicit Handle(const int i) : index(i){};
    int index;
    friend BinaryHeap;
  };
...
private:
  struct Node {
    Node(const T &_object, Handle *_index) : object(_object), handle(_index){};
    T object;
    Handle *handle;
...
public:
  explicit BinaryHeap(int _maxSize, Comparator _comparator = Comparator())
      : m_maxSize(_maxSize),
        ...
        m_nbElements(0), m_comparator(std::move(_comparator)) {}
  ...
  BinaryHeap(const BinaryHeap &_o)
      : m_maxSize(_o.m_maxSize),
        m_array(static_cast<Node *>(malloc(sizeof(Node) * m_maxSize))),
        m_handle(static_cast<Handle *>(malloc(sizeof(Handle) * m_maxSize))),
        m_nbElements(_o.m_nbElements), m_comparator(_o.m_comparator) {
    for (int i = 0; i < m_maxSize; ++i) {
  ...
  BinaryHeap(BinaryHeap &&_bh) noexcept
      : m_maxSize(std::move(_bh.m_maxSize)), m_array(std::move(_bh.m_array)),
