...
class BinaryHeap {
	...
	struct Node
	{
		Node(const T& object, Handle* index) : object(object), handle(index) {};
		T object;
		Handle *handle;
	...
	void pop() {
		if(m_nbElements > 0) {
			...
			while(firstChild < m_nbElements) {
				int child = secondChild < m_nbElements ? (m_comparator(m_array[firstChild].object, m_array[secondChild].object) ? firstChild : secondChild) : firstChild;
