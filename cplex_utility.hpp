...
int someProduct(std::tuple<int, Tr...>&& _pos, std::tuple<int, Tr...>&& _dim) {
    return someProduct(std::make_tuple(std::get<Tr...>(_pos)), std::make_tuple(std::get<Tr...>(_dim))) 
        * std::get<0>(_dim) + std::get<0>(_pos);
}
...
public:
    ...
    IloWrapper(IloEnv _env, IloType&& _val, T&&... args) :
        m_dimSize({args...}),
        m_array(_env, std::accumulate(m_dimSize.begin(), m_dimSize.end(), 1, std::multiplies<int>()), _val)
    {
    ...
    operator IloArray<IloType>&() {
    	return m_array;
    }

    operator const IloArray<IloType>&() const {
    	return m_array;
    }
