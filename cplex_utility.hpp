...
template <> int someProduct(std::tuple<int> &&_v1, std::tuple<int> && /*_v2*/) {
  return std::get<0>(_v1);
}

template <typename IloType, int nbDim> class IloWrapper {
public:
  template <typename... T>
  ...
  template <typename... T> IloType &operator[](std::tuple<T...> &&_tuple) {
