...
std::vector<int> getKMedoids(const int _k, const Matrix<double>& _dist, const int _tMax = 100) {
    assert(_dist.size1() == _dist.size2());
    std::vector<std::vector<int>> clusters(_k);
    // Init medoids
    std::vector<int> medoids = MyRandom::getInstance().getKShuffled(_k, 0, _dist.size2() - 1);
    bool changed = true;
    for (int t = 0; t < _tMax && changed; ++t) {
        ...
        // For each elements, find the closest medoid and assign the element to the cluster of the corresponding medoid
        ...
        for (int c = 0; c < static_cast<int>(clusters.size()); ++c) {
            ...
            int minDistInd = 0;
            double minDist = std::accumulate(clusters[c].begin(), clusters[c].end(), 0.0,
...
std::vector<int> getKMedoids(const int _k, const Matrix<double>& _dist, const std::vector<int>& _capas, const std::vector<double>& _charge, const int _tMax = 100) {
    ...
    for (int t = 0; t < _tMax && changed; ++t) {
        ...
        // For each element, find the closest medoid and assign the element to the cluster of the corresponding medoid
        for (int i = 0; i < _dist.size1(); ++i) {
            int minInd = 0;
            for (int j = 1; j < static_cast<int>(medoids.size()); ++j) {
                if (_dist(i, medoids[j]) < _dist(i, medoids[minInd]) && // Update closest medoid if distance is smaller..
