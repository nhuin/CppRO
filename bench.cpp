...
static std::vector<std::string> networks = {"germany50", "zib54", "ta2"};

static void BM_std_sort(benchmark::State& state) {
    while (state.KeepRunning()) {
        state.PauseTiming();
        std::vector<Integer> tab(state.range(0));
        std::generate(tab.begin(), tab.end(), [] { return MyRandom::getInstance().getIntUniform<Integer>(std::numeric_limits<Integer>::min(), std::numeric_limits<Integer>::max()); });
        state.ResumeTiming();
...
static void BM_bellmanford_v1(benchmark::State& state) {
    auto network = DiGraph::loadFromFile("./instances/" + networks[state.range(0)] + "_topo.txt");
    ShortestPathBellmanFord sh_path(std::get<0>(network));
    while (state.KeepRunning()) {
        Graph::Path path = sh_path.getShortestPath_v1(MyRandom::getInstance().getIntUniform<Integer>(0, std::get<0>(network).getOrder() - 1),
            MyRandom::getInstance().getIntUniform<Integer>(0, std::get<0>(network).getOrder() - 1));
    }
}
...
static void BM_bellmanford_v2(benchmark::State& state) {
    ...
    ShortestPathBellmanFord sh_path(std::get<0>(network));
    while (state.KeepRunning()) {
        Graph::Path path = sh_path.getShortestPath_v2(MyRandom::getInstance().getIntUniform<Integer>(0, std::get<0>(network).getOrder() - 1),
            MyRandom::getInstance().getIntUniform<Integer>(0, std::get<0>(network).getOrder() - 1));
    }
}
...
static void BM_bellmanford_YenFirst(benchmark::State& state) {
    ...
    ShortestPathBellmanFord sh_path(std::get<0>(network));
    while (state.KeepRunning()) {
        Graph::Path path = sh_path.getShortestPath_YenFirst(MyRandom::getInstance().getIntUniform<Integer>(0, std::get<0>(network).getOrder() - 1),
            MyRandom::getInstance().getIntUniform<Integer>(0, std::get<0>(network).getOrder() - 1));
    }
}
...
static void BM_bellmanford_YenFirst_doublevector(benchmark::State& state) {
    ...
    ShortestPathBellmanFord sh_path(std::get<0>(network));
    while (state.KeepRunning()) {
        Graph::Path path = sh_path.getShortestPath_YenFirst_doublevector(MyRandom::getInstance().getIntUniform<Integer>(0, std::get<0>(network).getOrder() - 1),
            MyRandom::getInstance().getIntUniform<Integer>(0, std::get<0>(network).getOrder() - 1));
    }
}
...
static void BM_bellmanford_YenFirst_partition(benchmark::State& state) {
    ...
    ShortestPathBellmanFord sh_path(std::get<0>(network));
    while (state.KeepRunning()) {
        Graph::Path path = sh_path.getShortestPath_YenFirst_partition(MyRandom::getInstance().getIntUniform<Integer>(0, std::get<0>(network).getOrder() - 1),
            MyRandom::getInstance().getIntUniform<Integer>(0, std::get<0>(network).getOrder() - 1));
    }
}
...
static void BM_bellmanford_YenSecond_partition(benchmark::State& state) {
    ...
    ShortestPathBellmanFord sh_path(std::get<0>(network));
    while (state.KeepRunning()) {
        Graph::Path path = sh_path.getShortestPath_YenSecond_partition(MyRandom::getInstance().getIntUniform<Integer>(0, std::get<0>(network).getOrder() - 1),
            MyRandom::getInstance().getIntUniform<Integer>(0, std::get<0>(network).getOrder() - 1));
    }
}
...
static void BM_bellmanford_v3(benchmark::State& state) {
    ...
    ShortestPathBellmanFord sh_path(std::get<0>(network));
    while (state.KeepRunning()) {
        Graph::Path path = sh_path.getShortestPath_v3(MyRandom::getInstance().getIntUniform<Integer>(0, std::get<0>(network).getOrder() - 1),
            MyRandom::getInstance().getIntUniform<Integer>(0, std::get<0>(network).getOrder() - 1));
    }
}
