...
inline void ZshCompletionOutput::version(CmdLineInterface& _cmd) {
    std::cout << _cmd.getVersion() << '\n';
}

inline void ZshCompletionOutput::usage(CmdLineInterface& _cmd) {
    std::vector<Arg*> argList = _cmd.getArgList();
    std::string progName = _cmd.getProgramName();
    std::string xversion = _cmd.getVersion();
    theDelimiter = _cmd.getDelimiter();
    basename(progName);

    std::cout << "#compdef " << progName << '\n'
              << '\n'
    ...
    inline void ZshCompletionOutput::failure(CmdLineInterface& /*unused*/, ArgException & e) {
        std::cout << e.what() << '\n';
    }
