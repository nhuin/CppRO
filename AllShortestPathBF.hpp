...
class AllShortestPathBellmanFord {
  public:
    explicit AllShortestPathBellmanFord(G& _graph)
        : m_graph(_graph)
        , m_distance([&]() {
            Matrix<double> distance(m_graph.getOrder(), m_graph.getOrder(), std::numeric_limits<int>::max());
        ...
        , m_parent([&]() {
            Matrix<Graph::Node> parent(m_graph.getOrder(), m_graph.getOrder(), -1);
            for (Graph::Node u = 0; u < m_graph.getOrder(); ++u) {
                parent(u, u) = u;
            }
            return parent;
    ...
    AllShortestPathBellmanFord& operator=(const AllShortestPathBellmanFord&) = default;
    AllShortestPathBellmanFord(AllShortestPathBellmanFord&&) noexcept = default;
    AllShortestPathBellmanFord& operator=(AllShortestPathBellmanFord&&) noexcept = default;
        ...
        for (Graph::Node s = 0; s < m_graph.getOrder(); ++s) {
            for (int i = 0; i < m_graph.getOrder(); ++i) {
                for (const auto& edge : edges) {
                    const double dist = m_distance(s, edge.first) + m_graph.getEdgeWeight(edge);
    ...
    const Matrix<double>& getDistance() const {
        return m_distance;
    }
